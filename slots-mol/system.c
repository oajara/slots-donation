...
int  main ( int argc, char *argv[] )
{
	...
	/*------------------------------------
	* initialize DRVS info struct
 	*------------------------------------*/	
	nodeid = mnx_getdrvsinfo(&drvs);
	if(nodeid < 0 )
	 ...
	 *------------------------------------*/
	vm_ptr = &vmu;
	rcode = mnx_getvminfo(vmu.vm_vmid, &vmu);	
	if(rcode) {
		TASKDEBUG("local_nodeid=%d\n", local_nodeid);
		do {		
			nodeid = VM_init(argc, argv);
			vmid   = vmu.vm_vmid;
			if( nodeid < 0 && nodeid != EMOLEXIST) ERROR_EXIT(nodeid);
			if( nodeid == EMOLEXIST) {
				rcode = mnx_vm_end(vmid);
...
/*-------------------------- FORK TO OTHER TASKS/SERVERS --------------------------------
	if( (pm_pid = fork()) == 0){		
		printf("Binding PM to VM %d with p_nr=%d pid=%d\n", vmid, PM_PROC_NR,  pm_pid);
    		sys_ep = mnx_bind(vmid, PM_PROC_NR);
...
-----------------------------------------------------------------------------------------*/
	...
	rcode = init_systask(vmid);
	if(rcode) ERROR_EXIT(rcode);

	rcode = init_clock(vmid);
	if (rcode ) ERROR_EXIT(rcode);
			...
			if(rcode) {
				switch(rcode){  /* Auto Unbind the failed process */
					...
					case	EMOLDEADSRCDST:
						...
						rcode = mnx_getprocinfo(vm_ptr->vm_vmid, m.m_source, proc_ptr);	
						...
						rcode = mnx_unbind(vm_ptr->vm_vmid, m.m_source);
						if( rcode < 0) ERROR_RETURN(rcode );
#ifdef ALLOC_LOCAL_TABLE 			
						rcode = mnx_getprocinfo(vm_ptr->vm_vmid, m.m_source, proc_ptr);
						if( rcode < 0) ERROR_RETURN(rcode );
...
static char debug_path[MNX_PATH_MAX];
	...
	/*------------------------------------
	 * Bind SYSTEM to the VM
	 *------------------------------------*/
	sys_pid = getpid();
	sys_nr = SYSTASK(local_nodeid);
   	TASKDEBUG( "Binding to VM %d with sys_nr=%d sys_pid=%d\n", vmid, sys_nr, sys_pid);
   	sys_ep = mnx_bind(vmid, sys_nr );
...
//  map(SYS_IOPENABLE, do_iopenable); 	/* Enable I/O */
	...
	* Alloc memory for process table 
 	*------------------------------------*/
#ifdef ALLOC_LOCAL_TABLE	
//	proc = (proc_usr_t *) malloc(sizeof(proc_usr_t) * (vmu.vm_nr_procs+vmu.vm_nr_tasks));
	posix_memalign( (void**) &proc, getpagesize(), sizeof(proc_usr_t) * (vmu.vm_nr_procs+vmu.vm_nr_tasks));
	if( proc == NULL) return (EMOLNOMEM);
  	TASKDEBUG( "Allocated space for nr_procs=%d + nr_tasks=%d processes\n",
		vmu.vm_nr_procs,vmu.vm_nr_tasks); 
#else /* ALLOC_LOCAL_TABLE */	
	/*------------------------------------
	* Map kernel process table 
 	*------------------------------------*/
	sprintf( debug_path, "/sys/kernel/debug/drvs/%s/procs",vm_ptr->vm_name);
	...
	kproc = mmap(NULL, ((vm_ptr->vm_nr_tasks + vm_ptr->vm_nr_procs) * drvs_ptr->d_size_proc) 
	...
	for (i = -vm_ptr->vm_nr_tasks; i < (vm_ptr->vm_nr_procs); i++) {
	...
	* Alloc memory for slot descriptor table 
 	*------------------------------------*/
	posix_memalign( (void**) &slot, getpagesize(),  sizeof(slot_t)  * (vmu.vm_nr_procs+vmu.vm_nr_tasks));
	...
	* Alloc memory for temporal slot descriptor table used on merging
 	*------------------------------------*/
	posix_memalign( (void**) &slot_merge, getpagesize(),  sizeof(slot_t)  * (vmu.vm_nr_procs+vmu.vm_nr_tasks));
	...
	* Alloc memory for global status  
 	*------------------------------------*/
	sts_size = sizeof(message) + sizeof(VM_usr_t) +
...
//	sts_ptr  = malloc(sts_size);
	...
	msg_ptr = sts_ptr;
	memcpy( (void *) sts_ptr + sizeof(message) , &vmu, sizeof(VM_usr_t));
	v_ptr = (void *) sts_ptr + sizeof(message);
	TASKDEBUG( VM_USR_FORMAT, VM_USR_FIELDS(vm_ptr));
	TASKDEBUG( VM_USR_FORMAT, VM_USR_FIELDS(v_ptr));
	rst_ptr  = (char *) msg_ptr + sizeof(message) + sizeof(VM_usr_t);
	...
	* Calculate slots limits 
	...
	/* total VM  slots for user space processes  */
	...
	/* min_owned_slots	= (total_slots/vm_ptr->vm_nr_nodes)+1;  si hacemos min_owned_slots * nr_nodes  > total_slots */
	min_owned_slots	= (total_slots/vm_ptr->vm_nr_nodes); /* heuristics  */
	 ...
	 * group name = vm_ptr->vm_name 
	 * user name = node_ptr->n_nodeid
	 *------------------------------------*/
	if( vm_ptr->vm_nr_nodes > 1) {
		/*------------------------------------
		...
		*------------------------------------*/
		...
		TASKDEBUG("Local node is the only VM node to start\n");
		active_nodes = 1;
		init_nodes = 1;
	...
	if( vm_ptr->vm_nr_nodes > 1) {
		...
		for(i=0; i < drvs_ptr->d_nr_nodes; i++){			/* all slot owners are initialized */
	...
	* Alloc memory for priv table 
 	*------------------------------------*/
//	priv = (priv_usr_t *) malloc(sizeof(priv_usr_t) * (vmu.vm_nr_procs+vmu.vm_nr_tasks));
	posix_memalign( (void**) &priv, getpagesize(), sizeof(priv_usr_t) * (vmu.vm_nr_procs+vmu.vm_nr_tasks));
	...
	* Initialize priv table 
	...
	for (i = 0; i < (vm_ptr->vm_nr_tasks + vm_ptr->vm_nr_procs); i++) {
		priv_ptr = &priv[i];
		rcode  = mnx_getpriv(vmu.vm_vmid, (i-vm_ptr->vm_nr_tasks), priv_ptr);
...
//TASKDEBUG(PRIV_USR_FORMAT, PRIV_USR_FIELDS(priv_ptr)); 
	...
	* Set SYSTASK/CLOCK privileges 
 	*------------------------------------*/
	priv_ptr = &priv[SYSTEM+vm_ptr->vm_nr_tasks];
	priv_ptr->s_level = SYSTEM_PRIV;
	priv_ptr->s_warn = NONE;
	rcode  = mnx_setpriv(vmu.vm_vmid, SYSTEM, priv_ptr);
	if( rcode < 0 && rcode != EMOLDSTDIED ) ERROR_RETURN(rcode);		

	priv_ptr = &priv[CLOCK+vm_ptr->vm_nr_tasks];
	priv_ptr->s_level = KERNEL_PRIV;
	priv_ptr->s_warn = NONE;
	rcode  = mnx_setpriv(vmu.vm_vmid, CLOCK, priv_ptr);
	if( rcode < 0 && rcode != EMOLDSTDIED ) ERROR_RETURN(rcode);		
...
{	
	...
	/* BIND REMOTE SYSTASK */
	TASKDEBUG("node=%d SYSTASK(%d)=%d vm_ptr->vm_nr_tasks=%d bm_init=%x\n", 
			node, node, SYSTASK(node), vm_ptr->vm_nr_tasks, bm_init);	
	rcode = mnx_rmtbind(vm_ptr->vm_vmid,
				"systask",
				SYSTASK(node),
				node);			
...
#ifdef ALLOC_LOCAL_TABLE 			
	proc_ptr = &proc[SYSTASK(node)+vm_ptr->vm_nr_tasks];
	rcode = mnx_getprocinfo(vm_ptr->vm_vmid, SYSTASK(node), proc_ptr);
	if( rcode < EMOLERRCODE ) 	ERROR_RETURN(rcode);
#else /* ALLOC_LOCAL_TABLE */			
	...
	sp = &slot[SYSTASK(node)+vm_ptr->vm_nr_tasks];
...
{	
...
#ifdef ALLOC_LOCAL_TABLE 			
	proc_ptr = &proc[_ENDPOINT_P(endpoint)+vm_ptr->vm_nr_tasks];
	rcode = mnx_getprocinfo(vm_ptr->vm_vmid, endpoint, proc_ptr);
	if( rcode < 0) 	ERROR_RETURN(rcode);
#else /* ALLOC_LOCAL_TABLE */			
	...
	TASKDEBUG(PROC_USR_FORMAT,PROC_USR_FIELDS(proc_ptr));
	rcode = mnx_unbind(vm_ptr->vm_vmid,endpoint);
	TASKDEBUG(PROC_USR_FORMAT,PROC_USR_FIELDS(proc_ptr));
	
	sp = &slot[_ENDPOINT_P(endpoint)+vm_ptr->vm_nr_tasks];
	sp->s_endpoint = NONE;
	sp->s_flags = 0;
