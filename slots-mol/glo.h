...
EXTERN VM_usr_t vmu, vmu_primary, *vm_ptr, *vmf_ptr;
...
EXTERN int active_nodes;	/* current number of nodes that conform the VM (0 < active_nodes <= vm_nr_nodes)*/
EXTERN int init_nodes;		/* current number of nodes that conform the VM (0 <  init_nodes <= active_nodes <= vm_nr_nodes)*/
...
EXTERN int total_slots;		/*Total number of slots for non system processes for all VM nodes */
...
EXTERN	char *msg_ptr;			/* pointer to the header message before VM status and GST */
